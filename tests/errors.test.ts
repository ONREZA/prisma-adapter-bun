 ...
 */
function makePgError(pgCode: string, opts: Partial<ConstructorParameters<typeof SQL.PostgresError>[1]> = {}): SQL.PostgresError {
...
describe('convertDriverError', () => {
  describe('PostgreSQL error codes', () => {
    ...
    test('unknown code -> postgres passthrough', () => {
      const err = makePgError('99999', { severity: 'FATAL', hint: 'some hint' })
      const result = convertDriverError(err)
